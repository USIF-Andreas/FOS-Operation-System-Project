...
void env_free(struct Env *e)
{
...
#if USE_KHEAP
	struct WorkingSetElement *wse_abdo;
	while((wse_abdo = LIST_FIRST(&(e->page_WS_list))) != NULL)
	{
		unmap_frame(e->env_page_directory, wse_abdo->virtual_address);
		LIST_REMOVE(&(e->page_WS_list), wse_abdo);
		kfree(wse_abdo);
	}
	while((wse_abdo = LIST_FIRST(&(e->ActiveList))) != NULL)
	{
		unmap_frame(e->env_page_directory, wse_abdo->virtual_address);
		LIST_REMOVE(&(e->ActiveList), wse_abdo);
		kfree(wse_abdo);
	}
	while((wse_abdo = LIST_FIRST(&(e->SecondList))) != NULL)
	{
		unmap_frame(e->env_page_directory, wse_abdo->virtual_address);
		LIST_REMOVE(&(e->SecondList), wse_abdo);
		kfree(wse_abdo);
	}
	struct PageRefElement *pre_abdo;
	while((pre_abdo = LIST_FIRST(&(e->referenceStreamList))) != NULL)
	{
		LIST_REMOVE(&(e->referenceStreamList), pre_abdo);
		kfree(pre_abdo);
	}
#else
    ...
    struct Share *next_shr_abdo;
    void free_share(struct Share* ptrShare_abdo);
   
    bool wasHeld_abdo = holding_kspinlock(&(AllShares.shareslock));
    if (!wasHeld_abdo) {
        acquire_kspinlock(&(AllShares.shareslock));
    }
    shr_abdo = LIST_FIRST(&(AllShares.shares_list));
    while (shr_abdo != NULL)
    {
        next_shr_abdo = LIST_NEXT(shr_abdo);
        if (shr_abdo->ownerID == e->env_id)
        {
            free_share(shr_abdo);
        }
        shr_abdo = next_shr_abdo;
    }
	...
	for (int i = 0; i < PDX(USER_TOP); i++)
	{
		...
		{
			uint32 table_pa = e->env_page_directory[i] & ~0xFFF;
			struct FrameInfo *ptr_table_frame = to_frame_info(table_pa);
			e->env_page_directory[i] = 0;
			if(ptr_table_frame)
				decrement_references(ptr_table_frame);
		}
	}
	...
	/*========================*/
	// If e is the current env, we can't free the stack we are running on.
	if (e != get_cpu_proc())
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	cprintf("create_user_kern_stack(uint32* ptr_user_page_directory) ===> ");

		uint32* the_KU = kmalloc(KERNEL_STACK_SIZE);
		if(the_KU == NULL)
		{
			panic("NOT ENOUGH KERNEL STACK SPACE");
		}



	    uint32* MY_table=NULL;
	    if(get_page_table(ptr_user_page_directory,(uint32)the_KU, &MY_table) == TABLE_IN_MEMORY){



	    uint32 PS=MY_table[PTX(the_KU)];
	    MY_table[PTX(the_KU)] = PS&(~PERM_PRESENT);
	    }
