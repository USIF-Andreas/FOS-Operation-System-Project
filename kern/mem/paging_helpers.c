...
inline int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
	//TODO: PRACTICE: fill this function.
		...
		uint32* Ppage_T ;
		int R = get_page_table(directory, virtual_address, &Ppage_T);
		if (Ppage_T != NULL)
		{
			return (Ppage_T[PTX(virtual_address)] & 0x00000FFF);
		}
		else
		{
			return -1;
...
inline void pt_clear_page_table_entry(uint32* directory, uint32 virtual_address)
{
    ...
    if (ptr_page_table == NULL)
        panic("pt_clear_page_table_entry() called with invalid virtual address  ------> Page table does NOT exist!");
