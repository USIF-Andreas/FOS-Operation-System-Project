...
struct Env* fos_scheduler_PRIRR()
{
	...
	//Your code is here
	struct Env *nextS= NULL;
	struct Env *curS = get_cpu_proc();
	if (curS != NULL) {
        curS->env_status = ENV_READY;
        sched_insert_ready(curS);
    }
	int y=0;
     for (int p = 0; p < num_of_ready_queues; p++) {
        if (queue_size(&(ProcessQueues.env_ready_queues[p])) > 0) {
            nextS = dequeue(&(ProcessQueues.env_ready_queues[p]));
            if (nextS != NULL) {
            	  nextS->waiting_time=0;
            	  if(!y)
            	  { nextS->env_status = ENV_RUNNING;
...
void clock_interrupt_handler(struct Trapframe* tf)
{
		...
		if (isSchedMethodPRIRR())
	{
		...
		//Your code is here
			        ...
			        {
			            struct Env_Queue* q = &(ProcessQueues.env_ready_queues[S]);
			            struct Env* e = q->lh_first;

			            while (e != NULL)
			            {
			                struct Env* next =  e->prev_next_info.le_next; // save next
			                e->waiting_time++;
			                e = next;
			            }
			        }
		   for (int S = 1; S < num_of_ready_queues; S++)
		    {
		        struct Env_Queue* q = &(ProcessQueues.env_ready_queues[S]);
		        struct Env* e=q->lh_first;
			//acquire_kspinlock(&ProcessQueues.qlock);
		        ...
		        while (e != NULL)
		        {
		        	 struct Env* next = e->prev_next_info.le_next;
		        	// e->waiting_time++;

		        	if(e->waiting_time>=limit_starvation){
		        	...
		        	//	q->size--;
		        	    e->waiting_time = 0;
		        	   // struct Env_Queue* higherQ = &(ProcessQueues.env_ready_queues[e->priority]);
...
void update_WS_time_stamps()
{
    ...
    while(curr!=NULL){
        if(curr->empty){curr=LIST_NEXT(curr);continue;}

        uint32 perms=pt_get_page_permissions(e->env_page_directory,curr->virtual_address);
        uint32 uB = 0;
        if (perms & PERM_USED)
            uB = 1;
        curr->time_stamp=(curr->time_stamp>>1)|(uB << 31);
        pt_set_page_permissions(e->env_page_directory,curr->virtual_address,0,PERM_USED);
        curr=LIST_NEXT(curr);}
#endif
    //Comment the following line
